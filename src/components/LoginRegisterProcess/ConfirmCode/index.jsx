...
import { setAuthCookie } from 'lib/Storage';
...
function ConfirmCode() {
  ...
  const handleNext = async (e) => {
    ...
    try {
      const {
        data: { data },
      } = await verifyUser(+verificationCode, phoneNumber);
      if (data?.jwt) {
  ...
  const resendHandler = async () => {
    setSending(true);
    try {
      const { data } = await resendCode(phoneNumber);
      if (data?.success) {
        setSending(false);
      }
    } catch (err) {
          ...
          >
            ...
            <div className={classes.resendcode}>
              <Typography
                ...
                color="textSecondary"
              >
                {sending ? 'درحال ارسال کد فعال سازی...' : 'ارسال مجدد کد فعال سازی؟'}
              </Typography>
            </div>
